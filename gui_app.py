...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.configure(fg_color="#1A0B2E")
        ...
        # Sidebar
        self.sidebar_frame = ctk.CTkFrame(self, width=200, corner_radius=0, fg_color="#240F3D")
        self.sidebar_frame.grid(row=0, column=0, sticky="nsew")
        self.sidebar_frame.grid_rowconfigure(5, weight=1)
        
        # Sidebar Border
        self.sidebar_border = ctk.CTkFrame(self, width=2, fg_color="#9D00FF", corner_radius=0)
        self.sidebar_border.grid(row=0, column=0, sticky="nse")

        self.logo_label = ctk.CTkLabel(self.sidebar_frame, text="🛡️ VRC Mod Tool", font=ctk.CTkFont(size=20, weight="bold"), text_color="#C55CFF")
        ...
        # Console Panel
        self.console_frame = ctk.CTkFrame(self, corner_radius=0, fg_color="#0A0A0A", border_width=1, border_color="#4D0080")
        self.console_frame.grid(row=1, column=0, columnspan=2, sticky="nsew")
        
        self.console_header = ctk.CTkFrame(self.console_frame, corner_radius=0, fg_color="#240F3D", height=24)
        ...
        ctk.CTkButton(self.console_header, text="Clear", font=("Consolas", 10, "bold"), width=50, height=20, fg_color="transparent", hover_color="#2E144D", text_color="#00FF41", command=self.clear_console).pack(side="right", padx=10)
    ...
    def init_services(self):
        ...
        self.update_status("Connecting to API...", "#F2E6FF")
        ...
        try:
            ...
            self.update_status("Connecting to VRCX...", "#F2E6FF")
    ...
    def update_status(self, text, color="#F2E6FF"):
    ...
    def show_dashboard(self):
        ...
        title = ctk.CTkLabel(self.main_frame, text="Dashboard", font=ctk.CTkFont(size=24, weight="bold"), text_color="#F2E6FF")
        title.pack(pady=20, padx=20, anchor="w")
        
        info_frame = ctk.CTkFrame(self.main_frame, fg_color="#2E144D", corner_radius=10)
        info_frame.pack(pady=10, padx=20, fill="x")
        
        ctk.CTkLabel(info_frame, text=f"Logged in as: {self.display_name}", font=("Arial", 16), text_color="#F2E6FF").pack(pady=10, padx=10, anchor="w")
        ctk.CTkLabel(info_frame, text=f"Managing Group: {self.group_name}", font=("Arial", 16), text_color="#F2E6FF").pack(pady=10, padx=10, anchor="w")
    ...
    def show_vrcx(self):
        self.clear_main_frame()
        title = ctk.CTkLabel(self.main_frame, text="VRCX Recent Players", font=ctk.CTkFont(size=24, weight="bold"), text_color="#F2E6FF")
        ...
        btn_export = ctk.CTkButton(btn_frame, text="Export to CSV", fg_color="#9D00FF", hover_color="#7A00CC", text_color="#F2E6FF", command=self.export_vrcx_csv)
        btn_export.pack(side="left")
        
        self.vrcx_list_frame = ctk.CTkScrollableFrame(self.main_frame, fg_color="#2E144D", corner_radius=10)
        self.vrcx_list_frame.pack(fill="both", expand=True, padx=20, pady=(0, 20))
        
        # Header
        header_font = ctk.CTkFont(weight="bold")
        ctk.CTkLabel(self.vrcx_list_frame, text="Display Name", font=header_font, text_color="#D580FF", width=200, anchor="w").grid(row=0, column=0, padx=10, pady=5)
        ctk.CTkLabel(self.vrcx_list_frame, text="User ID", font=header_font, text_color="#D580FF", width=300, anchor="w").grid(row=0, column=1, padx=10, pady=5)
        ctk.CTkLabel(self.vrcx_list_frame, text="Last Seen", font=header_font, text_color="#D580FF", width=150, anchor="w").grid(row=0, column=2, padx=10, pady=5)
    ...
    def _load_vrcx_data(self):
        self.update_status("Loading VRCX data...", "#F2E6FF")
    ...
    def _render_vrcx_data(self, recent):
        self.update_status("✅ Connected", "#00FF41")
        if not recent:
            ctk.CTkLabel(self.vrcx_list_frame, text="No players found in the last 24h.", text_color="#F2E6FF").grid(row=1, column=0, columnspan=3, pady=20)
            self.log_to_console("VRCX loaded: 0 players found.")
            return
            
        for i, player in enumerate(recent[:100], start=1):
            ctk.CTkLabel(self.vrcx_list_frame, text=player["displayName"], text_color="#F2E6FF", width=200, anchor="w").grid(row=i, column=0, padx=10, pady=2)
            id_entry = ctk.CTkEntry(self.vrcx_list_frame, width=300, fg_color="transparent", text_color="#F2E6FF", border_width=0)
            ...
            ctk.CTkLabel(self.vrcx_list_frame, text=ts, text_color="#F2E6FF", width=150, anchor="w").grid(row=i, column=2, padx=10, pady=2)
    ...
    def show_moderation(self):
        self.clear_main_frame()
        title = ctk.CTkLabel(self.main_frame, text="Moderation Panel", font=ctk.CTkFont(size=24, weight="bold"), text_color="#F2E6FF")
        title.pack(pady=(20, 10), padx=20, anchor="w")
        
        # Single User Action Frame
        single_frame = ctk.CTkFrame(self.main_frame, fg_color="#2E144D", corner_radius=10)
        single_frame.pack(fill="x", padx=20, pady=10)
        
        ctk.CTkLabel(single_frame, text="User ID or Display Name:", font=ctk.CTkFont(weight="bold"), text_color="#F2E6FF").pack(pady=(10, 0), padx=10, anchor="w")
        
        self.mod_user_entry = ctk.CTkEntry(single_frame, width=400, placeholder_text="usr_... or Exact Name", fg_color="#1A0B2E", border_color="#9D00FF", text_color="#F2E6FF")
        self.mod_user_entry.pack(pady=10, padx=10, anchor="w")
        
        btn_row = ctk.CTkFrame(single_frame, fg_color="transparent")
        btn_row.pack(fill="x", padx=10, pady=(0, 10))
        
        ctk.CTkButton(btn_row, text="🔎 Search API", width=100, fg_color="#9D00FF", hover_color="#7A00CC", text_color="#F2E6FF", command=self.action_search).pack(side="left", padx=5)
        ...
        # Mass Ban Frame
        mass_frame = ctk.CTkFrame(self.main_frame, fg_color="#2E144D", corner_radius=10)
        mass_frame.pack(fill="both", expand=True, padx=20, pady=10)
        
        ctk.CTkLabel(mass_frame, text="Mass Ban (Comma-separated User IDs):", font=ctk.CTkFont(weight="bold"), text_color="#F2E6FF").pack(pady=(10, 0), padx=10, anchor="w")
        
        self.mass_ban_textbox = ctk.CTkTextbox(mass_frame, height=100, fg_color="#0A0A0A", text_color="#00FF41", font=("Consolas", 12), border_width=1, border_color="#9D00FF")
    ...
    def _async_search(self, query):
        self.update_status("Searching API...", "#F2E6FF")
        try:
    ...
    def show_logs(self):
        self.clear_main_frame()
        title = ctk.CTkLabel(self.main_frame, text="Logs & Bans", font=ctk.CTkFont(size=24, weight="bold"), text_color="#F2E6FF")
        title.pack(pady=(20, 10), padx=20, anchor="w")
        
        btn_frame = ctk.CTkFrame(self.main_frame, fg_color="transparent")
        btn_frame.pack(fill="x", padx=20, pady=5)
        
        ctk.CTkButton(btn_frame, text="Load Active Bans", fg_color="#9D00FF", hover_color="#7A00CC", text_color="#F2E6FF", command=lambda: threading.Thread(target=self._load_bans, daemon=True).start()).pack(side="left", padx=5)
        ctk.CTkButton(btn_frame, text="Load Audit Logs", fg_color="#9D00FF", hover_color="#7A00CC", text_color="#F2E6FF", command=lambda: threading.Thread(target=self._load_audit, daemon=True).start()).pack(side="left", padx=5)
        
        self.log_list_frame = ctk.CTkScrollableFrame(self.main_frame, fg_color="#2E144D", corner_radius=10)
        self.log_list_frame.pack(fill="both", expand=True, padx=20, pady=(10, 20))
        self.log_to_console("Logs & Bans panel loaded.")
        
    def _load_bans(self):
        self.update_status("Loading Active Bans...", "#F2E6FF")
    ...
    def _render_bans(self, bans):
        ...
        if not bans:
            ctk.CTkLabel(self.log_list_frame, text="No active bans found.", text_color="#F2E6FF").pack(pady=20)
            self.log_to_console("LOGS > 0 active bans found.")
            return
            
        header_font = ctk.CTkFont(weight="bold")
        ctk.CTkLabel(self.log_list_frame, text="Target Name / ID", font=header_font, text_color="#D580FF", width=300, anchor="w").grid(row=0, column=0, padx=10, pady=5)
        ctk.CTkLabel(self.log_list_frame, text="Banned At", font=header_font, text_color="#D580FF", width=200, anchor="w").grid(row=0, column=1, padx=10, pady=5)
        ...
        for i, ban in enumerate(bans, start=1):
            ...
            ctk.CTkLabel(self.log_list_frame, text=name, text_color="#F2E6FF", width=300, anchor="w").grid(row=i, column=0, padx=10, pady=2)
            ctk.CTkLabel(self.log_list_frame, text=banned_at, text_color="#F2E6FF", width=200, anchor="w").grid(row=i, column=1, padx=10, pady=2)
            
        self.log_to_console(f"LOGS > Loaded {len(bans)} active bans.")

    def _load_audit(self):
        self.update_status("Loading Audit Logs...", "#F2E6FF")
    ...
    def _render_audit(self, logs):
        ...
        if not logs:
            ctk.CTkLabel(self.log_list_frame, text="No audit logs found.", text_color="#F2E6FF").pack(pady=20)
            self.log_to_console("LOGS > 0 audit logs found.")
            return
            
        header_font = ctk.CTkFont(weight="bold")
        ctk.CTkLabel(self.log_list_frame, text="Time", font=header_font, text_color="#D580FF", width=150, anchor="w").grid(row=0, column=0, padx=5, pady=5)
        ctk.CTkLabel(self.log_list_frame, text="Actor", font=header_font, text_color="#D580FF", width=150, anchor="w").grid(row=0, column=1, padx=5, pady=5)
        ctk.CTkLabel(self.log_list_frame, text="Action", font=header_font, text_color="#D580FF", width=150, anchor="w").grid(row=0, column=2, padx=5, pady=5)
        ctk.CTkLabel(self.log_list_frame, text="Target", font=header_font, text_color="#D580FF", width=200, anchor="w").grid(row=0, column=3, padx=5, pady=5)
        ...
        for i, log in enumerate(logs, start=1):
            ...
            ctk.CTkLabel(self.log_list_frame, text=time_val, text_color="#F2E6FF", width=150, anchor="w").grid(row=i, column=0, padx=5, pady=2)
            ctk.CTkLabel(self.log_list_frame, text=actor, text_color="#F2E6FF", width=150, anchor="w").grid(row=i, column=1, padx=5, pady=2)
            ctk.CTkLabel(self.log_list_frame, text=action, text_color="#F2E6FF", width=150, anchor="w").grid(row=i, column=2, padx=5, pady=2)
            ctk.CTkLabel(self.log_list_frame, text=target, text_color="#F2E6FF", width=200, anchor="w").grid(row=i, column=3, padx=5, pady=2)
