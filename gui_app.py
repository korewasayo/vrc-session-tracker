...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.logo_label = ctk.CTkLabel(self.sidebar_frame, text="🛡️ VRC Mod Tool", font=ctk.CTkFont(size=20, weight="bold"))
        self.logo_label.grid(row=0, column=0, padx=20, pady=(20, 10))

        self.btn_dashboard = ctk.CTkButton(self.sidebar_frame, text="Dashboard", command=self.show_dashboard)
        self.btn_dashboard.grid(row=1, column=0, padx=20, pady=10)
        
        self.btn_vrcx = ctk.CTkButton(self.sidebar_frame, text="VRCX Players", command=self.show_vrcx)
        self.btn_vrcx.grid(row=2, column=0, padx=20, pady=10)
        
        self.btn_moderation = ctk.CTkButton(self.sidebar_frame, text="Moderation", command=self.show_moderation)
        self.btn_moderation.grid(row=3, column=0, padx=20, pady=10)

        self.btn_logs = ctk.CTkButton(self.sidebar_frame, text="Logs & Bans", command=self.show_logs)
        self.btn_logs.grid(row=4, column=0, padx=20, pady=10)

        # Status Label
        self.status_label = ctk.CTkLabel(self.sidebar_frame, text="Initializing...", text_color="yellow")
    ...
    def init_services(self):
        ...
        if not auth_cookie or not group_id:
            self.update_status("❌ Missing .env config", "red")
            return

        self.update_status("Connecting to API...", "yellow")
        try:
            self.api = VRChatAPI()
            ...
            self.update_status("Connecting to VRCX...", "yellow")
            try:
                ...
                self.vrcx = None
                print(f"VRCX Error: {e}")
                
            self.update_status("✅ Connected", "green")
            self.after(0, self.show_dashboard)
            
        except Exception as e:
            self.update_status("❌ API Connection Failed", "red")
    ...
    def show_dashboard(self):
        ...
        ctk.CTkLabel(info_frame, text=f"Logged in as: {self.display_name}", font=("Arial", 16)).pack(pady=10, padx=10, anchor="w")
        ctk.CTkLabel(info_frame, text=f"Managing Group: {self.group_name}", font=("Arial", 16)).pack(pady=10, padx=10, anchor="w")
        
        vrcx_status = "Connected" if self.vrcx else "Not Found"
        vrcx_color = "green" if self.vrcx else "red"
        ctk.CTkLabel(info_frame, text=f"VRCX Status: {vrcx_status}", font=("Arial", 16), text_color=vrcx_color).pack(pady=10, padx=10, anchor="w")

    def show_vrcx(self):
        self.clear_main_frame()
        title = ctk.CTkLabel(self.main_frame, text="VRCX Recent Players", font=ctk.CTkFont(size=24, weight="bold"))
        title.pack(pady=(20, 10), padx=20, anchor="w")
        
        if not self.vrcx:
            ctk.CTkLabel(self.main_frame, text="VRCX Database not found or not connected.", text_color="red").pack(pady=20)
            return
            
        btn_frame = ctk.CTkFrame(self.main_frame, fg_color="transparent")
        btn_frame.pack(fill="x", padx=20, pady=10)
        
        btn_export = ctk.CTkButton(btn_frame, text="Export to CSV", command=self.export_vrcx_csv)
        btn_export.pack(side="left")
        
        self.vrcx_list_frame = ctk.CTkScrollableFrame(self.main_frame)
        self.vrcx_list_frame.pack(fill="both", expand=True, padx=20, pady=(0, 20))
        
        # Header
        header_font = ctk.CTkFont(weight="bold")
        ctk.CTkLabel(self.vrcx_list_frame, text="Display Name", font=header_font, width=200, anchor="w").grid(row=0, column=0, padx=10, pady=5)
        ctk.CTkLabel(self.vrcx_list_frame, text="User ID", font=header_font, width=300, anchor="w").grid(row=0, column=1, padx=10, pady=5)
        ctk.CTkLabel(self.vrcx_list_frame, text="Last Seen", font=header_font, width=150, anchor="w").grid(row=0, column=2, padx=10, pady=5)
        
        threading.Thread(target=self._load_vrcx_data, daemon=True).start()

    def _load_vrcx_data(self):
        self.update_status("Loading VRCX data...", "yellow")
    ...
    def _render_vrcx_data(self, recent):
        self.update_status("✅ Connected", "green")
        if not recent:
            ctk.CTkLabel(self.vrcx_list_frame, text="No players found in the last 24h.").grid(row=1, column=0, columnspan=3, pady=20)
            return
            
        for i, player in enumerate(recent[:100], start=1):
            ctk.CTkLabel(self.vrcx_list_frame, text=player["displayName"], width=200, anchor="w").grid(row=i, column=0, padx=10, pady=2)
            id_entry = ctk.CTkEntry(self.vrcx_list_frame, width=300, fg_color="transparent", border_width=0)
            ...
            ctk.CTkLabel(self.vrcx_list_frame, text=ts, width=150, anchor="w").grid(row=i, column=2, padx=10, pady=2)
    ...
    def show_moderation(self):
        self.clear_main_frame()
        title = ctk.CTkLabel(self.main_frame, text="Moderation Panel", font=ctk.CTkFont(size=24, weight="bold"))
        title.pack(pady=(20, 10), padx=20, anchor="w")
        
        # Single User Action Frame
        single_frame = ctk.CTkFrame(self.main_frame)
        single_frame.pack(fill="x", padx=20, pady=10)
        
        ctk.CTkLabel(single_frame, text="User ID or Display Name:", font=ctk.CTkFont(weight="bold")).pack(pady=(10, 0), padx=10, anchor="w")
        
        self.mod_user_entry = ctk.CTkEntry(single_frame, width=400, placeholder_text="usr_... or Exact Name")
        self.mod_user_entry.pack(pady=10, padx=10, anchor="w")
        
        btn_row = ctk.CTkFrame(single_frame, fg_color="transparent")
        btn_row.pack(fill="x", padx=10, pady=(0, 10))
        
        ctk.CTkButton(btn_row, text="🔎 Search API", width=100, command=self.action_search).pack(side="left", padx=5)
        ctk.CTkButton(btn_row, text="🚫 Ban", width=100, fg_color="#C62828", hover_color="#B71C1C", command=self.action_ban).pack(side="left", padx=5)
        ctk.CTkButton(btn_row, text="👢 Kick", width=100, fg_color="#E65100", hover_color="#E65100", command=self.action_kick).pack(side="left", padx=5)
        ctk.CTkButton(btn_row, text="✅ Unban", width=100, fg_color="#2E7D32", hover_color="#1B5E20", command=self.action_unban).pack(side="left", padx=5)
        
        # Mass Ban Frame
        mass_frame = ctk.CTkFrame(self.main_frame)
        mass_frame.pack(fill="both", expand=True, padx=20, pady=10)
        
        ctk.CTkLabel(mass_frame, text="Mass Ban (Comma-separated User IDs):", font=ctk.CTkFont(weight="bold")).pack(pady=(10, 0), padx=10, anchor="w")
        
        self.mass_ban_textbox = ctk.CTkTextbox(mass_frame, height=100)
        self.mass_ban_textbox.pack(fill="both", expand=True, padx=10, pady=10)
        
        ctk.CTkButton(mass_frame, text="📋 Execute Mass Ban", fg_color="#C62828", hover_color="#B71C1C", command=self.action_mass_ban).pack(pady=(0, 10), padx=10, anchor="e")
    ...
    def _async_search(self, query):
        self.update_status("Searching API...", "yellow")
        try:
    ...
    def _async_ban(self, target_id, target_name):
        self.update_status(f"Banning {target_id}...", "yellow")
        try:
    ...
    def _async_kick(self, target_id, target_name):
        self.update_status(f"Kicking {target_id}...", "yellow")
        try:
    ...
    def _async_unban(self, target_id):
        self.update_status(f"Unbanning {target_id}...", "yellow")
        try:
    ...
    def _async_mass_ban(self, valid_ids):
        ...
        for uid in valid_ids:
            self.update_status(f"Banning {uid}...", "yellow")
            try:
    ...
    def show_logs(self):
        self.clear_main_frame()
        title = ctk.CTkLabel(self.main_frame, text="Logs & Bans", font=ctk.CTkFont(size=24, weight="bold"))
        title.pack(pady=(20, 10), padx=20, anchor="w")
        
        btn_frame = ctk.CTkFrame(self.main_frame, fg_color="transparent")
        btn_frame.pack(fill="x", padx=20, pady=5)
        
        ctk.CTkButton(btn_frame, text="Load Active Bans", command=lambda: threading.Thread(target=self._load_bans, daemon=True).start()).pack(side="left", padx=5)
        ctk.CTkButton(btn_frame, text="Load Audit Logs", command=lambda: threading.Thread(target=self._load_audit, daemon=True).start()).pack(side="left", padx=5)
        
        self.log_list_frame = ctk.CTkScrollableFrame(self.main_frame)
        self.log_list_frame.pack(fill="both", expand=True, padx=20, pady=(10, 20))
        
    def _load_bans(self):
        self.update_status("Loading Active Bans...", "yellow")
        try:
    ...
    def _render_bans(self, bans):
        ...
        header_font = ctk.CTkFont(weight="bold")
        ctk.CTkLabel(self.log_list_frame, text="Target Name / ID", font=header_font, width=300, anchor="w").grid(row=0, column=0, padx=10, pady=5)
        ctk.CTkLabel(self.log_list_frame, text="Banned At", font=header_font, width=200, anchor="w").grid(row=0, column=1, padx=10, pady=5)
        ...
        for i, ban in enumerate(bans, start=1):
            ...
            ctk.CTkLabel(self.log_list_frame, text=name, width=300, anchor="w").grid(row=i, column=0, padx=10, pady=2)
            ctk.CTkLabel(self.log_list_frame, text=banned_at, width=200, anchor="w").grid(row=i, column=1, padx=10, pady=2)

    def _load_audit(self):
        self.update_status("Loading Audit Logs...", "yellow")
        try:
    ...
    def _render_audit(self, logs):
        ...
        header_font = ctk.CTkFont(weight="bold")
        ctk.CTkLabel(self.log_list_frame, text="Time", font=header_font, width=150, anchor="w").grid(row=0, column=0, padx=5, pady=5)
        ctk.CTkLabel(self.log_list_frame, text="Actor", font=header_font, width=150, anchor="w").grid(row=0, column=1, padx=5, pady=5)
        ctk.CTkLabel(self.log_list_frame, text="Action", font=header_font, width=150, anchor="w").grid(row=0, column=2, padx=5, pady=5)
        ctk.CTkLabel(self.log_list_frame, text="Target", font=header_font, width=200, anchor="w").grid(row=0, column=3, padx=5, pady=5)
        ...
        for i, log in enumerate(logs, start=1):
            ...
            ctk.CTkLabel(self.log_list_frame, text=time_val, width=150, anchor="w").grid(row=i, column=0, padx=5, pady=2)
            ctk.CTkLabel(self.log_list_frame, text=actor, width=150, anchor="w").grid(row=i, column=1, padx=5, pady=2)
            ctk.CTkLabel(self.log_list_frame, text=action, width=150, anchor="w").grid(row=i, column=2, padx=5, pady=2)
            ctk.CTkLabel(self.log_list_frame, text=target, width=200, anchor="w").grid(row=i, column=3, padx=5, pady=2)
